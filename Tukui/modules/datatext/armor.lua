	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() or (InCombatLockdown() and C["tooltip"].hidecombat ~= true) then
			...
			for i = 1, 4 do
				...
				GameTooltip:AddDoubleLine(T.cStart .. lv,format("%.2f", mitigation*100) .. "%",1,1,1,1,1,1)
				lv = lv - 1
			end
			if UnitLevel("target") > 0 and UnitLevel("target") < UnitLevel("player") then
				...
				GameTooltip:AddDoubleLine(T.cStart .. UnitLevel("target"),format("%.2f", mitigation*100) .. "%",1,1,1,1,1,1)
			end
			GameTooltip:Show()
		end
	end)
