...
if C["datatext"].avd and C["datatext"].avd > 0 then
	...
	local function Update(self)
		...
		if leveldifference >= 0 then
			...
			Text:SetText(T.cStart .. L.datatext_playeravd .. "|r" .. format("%.2f", avoidance))
		else
	...
	Stat:SetScript("OnEnter", function(self)
		-- if not InCombatLockdown() then
			...
			if targetlv > 1 then
				GameTooltip:AddDoubleLine(T.cStart .. L.datatext_avoidancebreakdown .. " (" .. L.datatext_lvl .. " " .. targetlv .. ")")
			elseif targetlv == -1 then
				GameTooltip:AddDoubleLine(T.cStart .. L.datatext_avoidancebreakdown .. " (" .. L.datatext_boss .. ")")
			else
				GameTooltip:AddDoubleLine(T.cStart .. L.datatext_avoidancebreakdown .. " (" .. L.datatext_lvl .. " " .. targetlv .. ")")
			end
			GameTooltip:AddDoubleLine(T.cStart .. L.datatext_miss,format("%.2f",MissChance) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(T.cStart .. L.datatext_dodge,format("%.2f",dodge) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(T.cStart .. L.datatext_parry,format("%.2f",parry) .. "%",1,1,1,  1,1,1)
			GameTooltip:AddDoubleLine(T.cStart .. L.datatext_block,format("%.2f",block) .. "%",1,1,1,  1,1,1)
			GameTooltip:Show()
		-- end
	end)
