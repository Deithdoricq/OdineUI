...
function T.LoadClassTimers(TukuiPlayer, TukuiTarget)
	assert(TukuiPlayer, "Classtimers failed to load, you need to specify a player and target frame, T.LoadClassTimers([player], [target])")
	assert(TukuiTarget, "Classtimers failed to load, you need to specify a player and target frame, T.LoadClassTimers([player], [target])")
			...
			CreateAuraBar = function( parent )
				...
				if ( bit.band( ICON_POSITION, 4 ) == 0 ) then		
					...
					if ICON_POSITION == 2 or ICON_POSITION == 3 then
						...
						iconbackdrop:SetPoint("TOPLEFT", icon, "TOPLEFT", T.Scale(-2), T.Scale(2))
						iconbackdrop:SetPoint("BOTTOMRIGHT", icon, "BOTTOMRIGHT", T.Scale(2), T.Scale(-2))
	...
	if ( LAYOUT == 1 ) then
		...
		dataSource:SetSortDirection( true );
		...
		local playerFrame = CreateAuraBarFrame( dataSource, TukuiPlayer );
		local yOffset = 6;
		local xOffset1 = 0
		local xOffset2 = -2
		...
		if TukuiPlayer.Debuffs then
			playerFrame:Point( "BOTTOMLEFT", TukuiPlayer.Debuffs, "TOPLEFT", xOffset1, yOffset );
			playerFrame:Point( "BOTTOMRIGHT", TukuiPlayer.Debuffs, "TOPRIGHT", xOffset2, yOffset );	
		elseif TukuiPlayer.Buffs then
	...
	elseif ( LAYOUT == 2 ) then
		...
		targetDataSource:SetSortDirection( true );
		playerDataSource:SetSortDirection( true );
	...
	elseif ( LAYOUT == 3 ) then
		...
		targetDataSource:SetSortDirection( true );
		playerDataSource:SetSortDirection( true );
		trinketDataSource:SetSortDirection( true );
	...
	elseif ( LAYOUT == 4 ) then
		...
		if TukuiTarget.Debuffs then
			targetFrame:Point( "BOTTOMLEFT", TukuiTarget.Debuffs, "TOPLEFT", xOffset1, yOffset );
			targetFrame:Point( "BOTTOMRIGHT", TukuiTarget.Debuffs, "TOPRIGHT", xOffset2, yOffset );
		elseif TukuiTarget.Buffs then
