...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self) if not button.cd.timer then self:SetScript("OnUpdate", nil) return end button.cd.timer.text:SetFont(C["media"].font,8,"THINOUTLINE") button.cd.timer.text:SetShadowColor(0, 0, 0, 0.4) end)
	button:Show()
end
...
local function UpdateObjects(frame)
	...
	if C["nameplate"].showhealth == true then
		frame.hp.level:SetPoint("RIGHT", frame.hp, "RIGHT", 2, 2)
	else
		frame.hp.level:SetPoint("RIGHT", frame.hp, "LEFT", -1, 2)
	end
	...
	if C["nameplate"].trackdebuffs == true or C["nameplate"].trackcc == true then
		...
		frame.icons:SetPoint("BOTTOMRIGHT", frame.hp, "TOPRIGHT", 0, 10+5)
...
local function SkinObjects(frame)
	...
	hp.level:SetFont(C["media"].font, 10, "OUTLINE")
	...
	if C["nameplate"].showhealth == true then
		...
		hp.value:SetFont(C["media"].font, 10, "OUTLINE")
	...
	name:SetFont(C["media"].font, 10, "OUTLINE")
...
local function CheckBlacklist(frame, ...)
	if T.PlateBlacklist[frame.name:GetText()] then
